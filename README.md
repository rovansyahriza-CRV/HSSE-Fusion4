...
- [x] **Management Walk Through (MWT)** -- kunjungan lapangan oleh manajemen/leadership buat liat langsung kondisi HSSE, ngobrol sama pekerja, dan kasih bukti visible leadership commitment. SENGAJA dibuat simpel: form submit tunggal (bukan checklist+skor kayak Inspeksi, bukan alur findings/CAPA berjenjang kayak Internal Audit), khusus Author **"Management Walkthrough"** yang boleh isi (scan QR/wajah/PIN) -- gak ada approval berjenjang. Field yang dicatat: Project/Kontrak, Tanggal Kunjungan, Area/Lokasi yang Dikunjungi, Pendamping (dinamis, nama+jabatan), Jumlah Pekerja yang Diajak Diskusi, Catatan Observasi (freeform, bukan daftar temuan terstruktur), Foto Dokumentasi. Realisasinya sekarang nyambung ke HSE Program (`get_hse_program`, KodeItem `MWT`) yang sebelumnya (`sql/031`) masih dikunci 0 karena modulnya belum dibangun. Lihat `sql/033_mwt_schema.sql`, `mwt-report.html`, `mwt-list.html`.
