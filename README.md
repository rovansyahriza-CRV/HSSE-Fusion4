...
- [x] **Management Review** -- `management-review.html` mencatat agenda, review inputs, keputusan, status review, action plan ter-normalisasi (PIC, due date, status, catatan), dan lampiran melalui Drive Bridge. Author **"Management Review"** diverifikasi lewat QR/PIN. Jalankan `sql/046_management_review_schema.sql` setelah migrasi HSE Program terakhir; review `Completed` otomatis masuk sebagai realisasi HSE Program. Frame **Follow Up Lintas Module (Belum Close)** mengambil item terbuka dari HSE Meeting, CERMAT, Incident, dan Internal Audit berdasarkan project, lalu item terpilih dapat dimasukkan ke Review Inputs. Halaman `management-review-list.html` menyediakan layar **Riwayat Management Review** (pencarian, filter project & status, kartu detail collapsible, peserta & absensi, action plan per topik, dan closing statement).
- Jika frame follow-up menampilkan error `Could not find the function ... in the schema cache`, jalankan `sql/047_management_review_followup_rpc.sql` di Supabase SQL Editor, lalu refresh halaman. File ini aman dijalankan ulang dan khusus memasang RPC follow-up tanpa mengulang migrasi 046.
- Jalankan `sql/048_management_review_action_topics.sql` setelah 047 untuk menambahkan kolom Topic pada Action Plan.
- Jalankan `sql/049_management_review_frame.sql` setelah 048. Frame Topik Pembahasan sekarang memakai 7 topik tetap (HSE Program, Incident & Near Miss, Audit/Inspeksi/Compliance dari CERMAT+Inspeksi, Partisipasi Pekerja dari MWT, lalu Risiko, Sumber Daya, dan Kesiapsiagaan Darurat diisi manual). Tiap baris topik punya tombol **+ Action Plan** yang menyalin ringkasan ke form Action Plan (PIC/Due Date/Status diisi manual sebelum klik Tambah Action).
- Jalankan `sql/050_management_review_frame_detail.sql` setelah 049 (aman dijalankan ulang, `CREATE OR REPLACE` di fungsi yang sama). Perbaikan: (1) `realisasiTotal` HSE Program sekarang benar-benar dihitung dari tabel laporan tiap modul (pola sama seperti `get_hse_program`), bukan 0 hardcoded; (2) tiap topik yang datanya otomatis (HSE Program, Incident & Near Miss, Audit/Inspeksi/Compliance, Partisipasi Pekerja) sekarang tampil sebagai **list per item** di frame, masing-masing dengan tombol **Lihat Detail** yang membuka popup berisi field lengkap sesuai laporan modul asalnya (kronologi, tindakan, PIC, tenggat, dst); (3) tombol **+ Action Plan** per topik sekarang membuka **popup** (Action Item/PIC/Due Date/Status/Catatan/Lampiran) yang langsung tersimpan lewat `save_management_review_action`, tidak lagi scroll ke form bawah.
- Jalankan `sql/051_management_review_closing_statement.sql` setelah 050 (aman dijalankan ulang). Menambah topik ke-8 **"Lain-lain"** (manual, sama seperti Risiko/Sumber Daya/Kesiapsiagaan Darurat) pada Frame Topik Pembahasan, kolom `ClosingStatement`, dan RPC `save_management_review_closing_statement`. Karena Action Plan kini selalu ditambahkan per item lewat popup **+ Action Plan** (baik dari frame topik maupun tombol Edit di daftar action tiap review), form tunggal "Action Plan" di bagian bawah halaman diganti jadi kartu **Closing Statement** -- pilih review yang sudah disimpan, isi kesimpulan/penutup review, lalu Simpan.
- Jalankan `sql/052_management_review_document_numbers.sql` setelah 051. Nomor dokumen Management Review sekarang unik per review dengan format `MR-YYYYMMDD-P<project>-R<reviewId>`; migrasi ini juga memberi suffix pada nomor lama yang duplikat dan memasang unique index.
- Jalankan `sql/053_management_review_report_sequence.sql` setelah 052. Suffix `R######` sekarang adalah nomor bundle laporan Management Review berurutan (misalnya setelah `R000016` menjadi `R000017`), bukan ID internal database. Sequence aman terhadap dua author yang membuat review bersamaan.
- Jalankan `sql/054_management_review_frame_report_links.sql` setelah 053 (aman dijalankan ulang). Menambah kolom `id` per item pada `incidentNearMiss`, `cermat.openList`, `inspeksi.openList`, dan `partisipasiPekerja.sesiList` sehingga Frame Topik Pembahasan bisa menampilkan tombol **📄 Lihat Laporan** langsung ke laporan/PDF modul asal per item (Incident, CERMAT, Inspeksi, MWT), sedangkan Topik 1 (HSE Program) menampilkan link **Lihat Laporan Lengkap** ke `hse-program-pdf.html?project_id=&tahun=`.
- Jalankan `sql/055_management_review_fix_registration_agenda.sql` setelah 054 (aman dijalankan ulang). Memperbaiki `Agenda` registrasi lama yang tersimpan tanpa suffix nomor bundle (`-R######`) akibat bug urutan generate nomor sebelumnya, supaya judul di Riwayat Management Review konsisten dengan `DocumentNo` finalnya. Tombol **Aktifkan Absensi** di `management-review.html` sekarang juga langsung menulis Agenda dengan nomor final, bukan preview.
- Halaman `management-review-list.html` sekarang otomatis mendeteksi Management Review yang masih `Draft`/`Scheduled`/`In Progress` dan mengubah tombol atas menjadi **"↩️ Lanjutkan Management Review yang Sedang Berjalan"** langsung ke review tersebut, alih-alih selalu membuka form kosong untuk review baru.
- [x] **QR absensi Management Review** -- setelah review disimpan, kartu sesi menampilkan QR besar, URL check-in + tombol Copy, dan link **Buka Tampilan Layar Besar** (`management-review-session.html`). Peserta scan dari HP ke `management-review-checkin.html?id=...`, lalu verifikasi diri dengan QR badge atau PIN. Daftar peserta dan jumlah hadir diperbarui berkala, duplicate `QrCodeId` ditolak, dan absensi ditutup otomatis saat review `Completed`/`Cancelled`.
